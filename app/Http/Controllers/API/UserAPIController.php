...
class UserAPIController extends Controller
{
    ...
    function login(Request $request)
    {
        try {
            ...
            $phone7 ='+7'.$request->input('phone');
            if (auth()->attempt(['phone' => $phone, 'password' => $request->input('password')])) {
            ...
            if (auth()->attempt(['phone' => $phone7, 'password' => $request->input('password')])) {
                // Authentication passed...
                $user = auth()->user();
                $user->device_token = $request->input('device_token', '');
                $user->save();
    ...
    function register(Request $request)
    {
        try {
            ...
            //if phone +7
            $user7 = User::where('phone', '=', $phone7)->first();
            if($user7 != null){
                $e = new Exception();
                return $this->sendError($e->getMessage() );
            }
            ...
            $user->email = Hash::make($request->input('phone'),['rounds' => 6,]) . '@mail.demo';
            $user->phone = $phone;
