...
use App\Criteria\CustomLimitOffsetCriteriaCriteria;
...
class OrderAPIController extends Controller
{
     ...
     */
    public function __construct(OrderRepository $orderRepo, FoodOrderRepository $foodOrderRepository, CartRepository $cartRepo, PaymentRepository $paymentRepo, NotificationRepository $notificationRepo, UserRepository $userRepository,CouponRepository $couponRepository)
    {
    ...
    public function index(Request $request)
    {
        try {
            ...
            $this->orderRepository->pushCriteria(new CustomLimitOffsetCriteriaCriteria($request));
    ...
    {
        $input = $request->all();
        if (isset($input['coupon']) && $input['coupon']['valid']) {
            $input['hint'] = empty($input['hint']) ? '' :  $input['hint'];
            $input['hint'] .= "\r\n".__('lang.applied_coupon', ["code" => $input['coupon']['code']]);
        ...
        try {
            ...
            if ($stripeToken->created > 0) {
                if ( empty($input['delivery_address_id']) || $input['delivery_address_id'] == 'null') {
    ...
    private function cashPayment(Request $request)
    {
        ...
        if (isset($input['coupon']) && $input['coupon']['valid']) {
            ...
            //save user used coupon

            $coupon = $this->couponRepository->findByField('code', $input['coupon']['code'])->first();
            if (collect($coupon)->isNotEmpty()) {

                $use = new CouponUsed(['user_id' => $input['user_id'], 'coupon_id' => $coupon->id]);
                $use->save();
            }
            //end save user used coupon
            $request->replace($input);
        }
        ...
        try {
            if ( empty($input['delivery_address_id']) || $input['delivery_address_id'] == 'null') {
                ...
                $order = $this->orderRepository->create(
                    $request->only('user_id', 'order_status_id', 'tax', 'hint','order_comment')
                );
            } else {

                $order = $this->orderRepository->create(
                    $request->only('user_id', 'order_status_id', 'tax', 'delivery_address_id', 'delivery_fee', 'hint','order_comment')
                );
            }
            foreach ($input['foods'] as $foodOrder) {
                $foodOrder['order_id'] = $order->id;
                $amount += $foodOrder['price'] * $foodOrder['quantity'];
               /* foreach ($foodOrder['extras'] as $extras) {

                    $extra = Extra::where('id','=', $extras)->first();
                    $amount += $extra->price;
                }*/
                $this->foodOrderRepository->create($foodOrder);
            }
            ...
            $this->orderRepository->update(['payment_id' => $payment->id,'readeble'=>0], $order->id);
            event(new \App\Events\BellNotification( $this->orderRepository->all()->where('readeble','=','0')->count()));
