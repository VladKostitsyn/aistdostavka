...
abstract class ParentOrderController extends Controller
{
    ...
    protected function createOrder()
    {
        if (isset($this->order->payment->status)) {
            ...
            try {
                if ($this->order->delivery_address_id < 1) {
                    $orders = (collect($this->order))->only('user_id', 'order_status_id', 'tax', 'hint')->toArray();
                } else {
                ...
                if ($this->coupon && $this->coupon->code != '') {
                    $orders['hint'] = empty($orders['hint']) ? '' :  $orders['hint'];
                    $orders['hint'] .= "\r\n".__('lang.applied_coupon', ["code" => $this->coupon->code]);
                }
    ...
    protected function calculateTotal(): float
    {
        ...
        if ( !$this->pickUpChoice ) {

            $minimum_sum_of_delivery_free = Restaurant::find($carts[0]->food->restaurant->id)->minimum_sum_of_delivery_free;
            if(is_null($minimum_sum_of_delivery_free)){

                $this->calculateDeliveryFee();

            }elseif (!($this->total >= $minimum_sum_of_delivery_free)){
