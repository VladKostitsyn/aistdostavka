...
class Extra extends Model implements HasMedia
{
    ...
    {
        return $this->belongsToMany(\App\Models\Food::class)->withPivot('extra_group_id');
    }

    public function fixFoods()
    {
        $foods = $this->foods;
        if (! count($foods)) {
            return false;
        ...
        foreach ($foods as $food) {
            DB::table('extra_food')
                ->where([['food_id', '=', $food->id], ['extra_id', '=', $this->id]])
                ->update(['extra_group_id' => $this->extra_group_id]);
        }
    }
