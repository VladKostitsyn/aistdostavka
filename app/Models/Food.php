...
class Food extends Model implements HasMedia
{
    ...
    {
        return $this->belongsToMany(\App\Models\Extra::class)->withPivot('extra_group_id');
    }
    ...
    public function fixExtras()
    {
        ...
        foreach ($extras as $extra) {
            DB::table('extra_food')
                ->where([['food_id', '=', $this->id], ['extra_id', '=', $extra->id]])
                ->update(['extra_group_id' => $extra->extra_group_id]);
        }
    }
    ...
    {
        return $this->belongsToMany(\App\Models\ExtraGroup::class,'extra_food');
    }
    ...
    public function applyCoupon($coupon): float
    {
        ...
        if (!isset($coupon) || !$coupon->enabled || strtotime($coupon->expires_at) < time()) {
            return $price;
        }

        $apply = false;

        foreach ($coupon->discountables as  $discountable) {
            if ($apply) break;
            switch ($discountable->discountable_type) {
                case 'App\Models\Food':
                    if ($this->id == $discountable->discountable_id) {
                        $apply = true;
                    }
                    break;
                case 'App\Models\Category':
                    if ($this->category && $this->category->id == $discountable->discountable_id) {
                        $apply = true;
                    }
                    break;
                case 'App\Models\Restaurant':
                    if ($this->restaurant && $this->restaurant->id == $discountable->discountable_id) {
                        $apply = true;
                    }
                    break;
            }
