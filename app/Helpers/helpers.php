...
//setting('default_currency')
    if ($modelObject[$attributeName] != null && strlen($modelObject[$attributeName]) > 0) {
        ...
        if (setting('currency_right', false) != false) {
            return $modelObject[$attributeName] . "<span>₸</span>";
        } else {
...
{
    if (setting('currency_right', false) != false) {
        return number_format((float)$price, 2, '.', '') . "<span>₸</span>";
    } else {
        return "<span>₸</span>" . number_format((float)$price, 2, '.', ' ');
    }
}
...
function addNewBage($column, $attributeName)
{
    if (isset($column)) {
            return "<span class='badge badge-success'>New</span>"."<span>" . '#'.$column->id . "</span>";
    }
}
/**
...
function getePriceWithExtras($array){
