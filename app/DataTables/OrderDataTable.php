...
use App\Repositories\Translit\TranslitRepository;
class OrderDataTable extends ServiceDataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('restaurant.name', function ($food) {
                $food = collect($food);
                if (array_key_exists('0', $food['foods'] )) {
                    return $food['foods'][0]['restaurant']['name'];
                }else {
            ...
            ->editColumn('user.phone', function ($food) {
              return isset($food->user->phone)  ? $food->user->phone : $food->user->custom_fields['phone']['view'];
            })
            ->editColumn('payment.price', function ($food) {

                $subtotal = 0;

                foreach ($food->foodOrders as $foodOrder) {
                ...
                $total = $subtotal + $food['delivery_fee'];
                $taxAmount = $total * $food['tax'] / 100;
                $total += $taxAmount;
...
/*            return $model->newQuery()->with("user")->with("orderStatus")->with('payment')->with("foods.restaurant");*/
            return $model->newQuery()->with("orderStatus")->with('payment')->with("foods.restaurant");
