...
<!-- Price Field -->
    ...
    <!-- 'Options' -->
    <div class="form-group row ">
        <div class="col-12" id="extra_groups">
            @include('foods.extra_groups')
        </div>
    </div>

    <!-- ExtraGroups Id Field -->
    <div class="form-group row extra-groups-container">
        {!! Form::label('', 'Группы опций',['class' => 'col-3 control-label text-right']) !!}
        <div class="col-9">
            {!! Form::select('', $extraGroups, count($extraGroupsSelected) ? $extraGroupsSelected->pluck('id') : [], ['class' => 'select2 form-control', 'multiple'=>'multiple']) !!}
            <div class="form-text text-muted">Выберите группу чтоб прикрепить опции к товару</div>
        </div>
    </div>
