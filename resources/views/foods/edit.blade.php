...
<script type="text/javascript">
    ...
    $('.extra-groups-container .select2').on("select2:select", function (e) {
      let extraGroupId = e.params.data.id,
          _token   = $('input[name="_token"]').get(0).value;
      $.ajax({
        method: "POST",
        url: "{!! url('foods', [$food->id, 'get-extra-group']) !!}",
        data: {
          egid: extraGroupId,
          _token: _token
        }
      ...
      .done(function( msg ) {
        $('#extra_groups').append(msg);
        $('.icheck input').iCheck('destroy').iCheck({
    ...
    $('.extra-groups-container .select2').on("select2:unselect", function (e) {
      $('#extra_group_' + e.params.data.id).remove();
    });
</script>
@endpush